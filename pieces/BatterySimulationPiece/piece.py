...
class BatterySimulationPiece(BasePiece):
    ...
    def piece_function(self, input_data: InputModel) -> OutputModel:
        ...
        try:
            ...
            merged = ld[["datetime", "load_kw"]].merge(
                sd[["datetime", "pv_kw"]], on="datetime", how="inner"
            )
            if len(merged) == 0:
                raise ValueError("No overlapping datetimes between load and solar CSV")
            df = (
                ld.merge(merged[["datetime"]], on="datetime", how="inner")
